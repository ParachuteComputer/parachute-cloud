...
workers/identity/   the OAuth issuer (authorize/token/DCR/JWKS/revocation on D1) —
                    ...
                    binding or a dev-log fallback) + optional TOTP 2FA (WebCrypto,
                    ported from the hub; enroll on /console/security). 86 tests
                    (42 issuer conformance + 29 console/ownership/throttle + 15
                    magic-link/TOTP).
...
cd workers/identity && bun run typecheck && bun x vitest run # 86
