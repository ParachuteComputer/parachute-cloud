...
`notes.parachute.computer/*` → **301** `https://my.parachute.computer/*` (path + query preserved).
...
every such request onto the live app. Points straight at the canonical `my.` origin
(my.-canonical Phase 1) rather than `app.`, avoiding a `notes.→app.→my.` double-hop now
that `app.` itself 301s to `my.`.
...
Verify: `curl -sI https://notes.parachute.computer/n/x` → `301` + `location: https://my.parachute.computer/n/x`.
