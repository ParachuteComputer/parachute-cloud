/**
 * Stripe SDK factory wired for the Workers runtime — harvested near-verbatim
 * from the old control plane's src/billing/stripe-client.ts (the dormant
 * Fly-era Worker), which already solved the two Workers-specific bits:
 *
 ...
 * (The `stripe` dependency itself is pinned EXACT in package.json — the
 * cloud#23 lesson: a caret range drifted the old control plane's types until
 * the root typecheck broke.)
 */
